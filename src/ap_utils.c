...
// rn this is the AP logic- making it a function in case the logic changes
...
int GetRemainingDungeonMissionChecks(enum dungeon_id dungeon, bool outlaw) {
    ...
    int expected;
    if (outlaw) {
        completed = CUSTOM_SAVE_AREA.missionStats.completedOutlaws[dungeon];
        if (IsDungeonPostDialga(dungeon)) {
            expected = missionMaxes.totalOutlawsLate;
        } else {
            expected = missionMaxes.totalOutlawsEarly;
        }
    } else {
        completed = CUSTOM_SAVE_AREA.missionStats.completedJobs[dungeon];
        if (IsDungeonPostDialga(dungeon)) {
